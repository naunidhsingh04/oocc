"""A throwaway PEP 669 (sys.monitoring) tracer used only to generate the
twelve golden fixtures in fixtures/*.trace.json. This is NOT services/executor
— it is deliberately small, synchronous, unsandboxed, and meant to run once
on a developer machine. It emits trace.schema.json-conforming JSON for real
program executions, including the deliberately-broken ones (infinite loop,
throwing program). See fixtures/README.md.

sys.monitoring is process-global, so two things matter for correctness:
  1. Every callback filters on `code.co_filename == USER_CODE_FILENAME` —
     otherwise every stdlib frame the traced program touches (including
     import machinery) gets traced too.
  2. All modules this file needs must already be imported before monitoring
     turns on, and monitoring must turn off before any of the tracer's own
     post-run code (e.g. `import traceback`) executes — otherwise that code
     runs *while monitored* and pollutes the trace with its own frames.
"""
...
class Tracer:
    ...
    def run(self, source: str, *, stdin: str = "") -> dict:
        ...
        sys.monitoring.use_tool_id(TOOL_ID, "oocc-fixture-tracer")
